        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <div class="row m-b-sm m-t-sm">
                    <div class="col-md-4">
                        <form action="{{ route('backend.master.desalist') }}" method="GET">
...
{{--                        <button class="btn btn-primary " type="button" onclick="createModal()"><i class="fa fa-plus"></i>&nbsp;Tambah Desa</button>--}}
{{--                    </div>--}}
                </div>
                ...
                <div class="project-list">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach ($desa as $data => $value)
                            <tr>
                                <td>{{ $desa->firstItem() + $data }}</td>
...
<form id="deleteform" action="{{ route('backend.master.desadelete') }}" method="POST" style="display: none;">
    @csrf
...
<script>
    ...
    $('#tambahModal  .modal-header .modal-title').html('Tambah Desa');
    $('#tambahModal  .modal-body').html('<i class="fa fa-cog fa-spin fa-2x fa-fw"></i> Loading...');
    $('#tambahModal  .modal-body').load('{{ route('backend.master.desacreateview') }}');
    $("#tambahModal").modal('show');
    }

    function editModal(pid) {
    $('#tambahModal  .modal-header .modal-title').html('Ubah Desa');
    $('#tambahModal  .modal-body').html('<i class="fa fa-cog fa-spin fa-2x fa-fw"></i> Loading...');
    $('#tambahModal  .modal-body').load('{{ route('backend.master.desaeditview') }}?pid=' + pid);
    $("#tambahModal").modal('show');
    }
    ...
    // swal({
    // title: "Hapus desa",
    //         text: "Anda yakin akan menghapus desa ini?",
    //         type: "warning",
